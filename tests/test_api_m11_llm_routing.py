...
def test_local_managed_vllm_activation_can_boot_service_and_load_lora(
    openai_client: TestClient,
    monkeypatch: pytest.MonkeyPatch,
):
    get_settings.cache_clear()
    monkeypatch.setattr("backend.services.vllm_client.httpx.Client", FakeVLLMClient)
    monkeypatch.setattr("backend.services.vllm_client.is_local_vllm_managed_for_settings", lambda settings: True)

    start_calls: list[bool] = []

    def fake_ensure_local_managed_vllm_started(*, enable_lora: bool, timeout: float | None = None):
        start_calls.append(enable_lora)
        return {"status": "started", "pid": 4321, "message": "started"}

    monkeypatch.setattr("backend.services.vllm_client.ensure_local_managed_vllm_started", fake_ensure_local_managed_vllm_started)
    FakeVLLMClient.reset()

    project_id = _create_project(openai_client, name="m11-local-managed")
    patch = openai_client.patch(f"/api/projects/{project_id}/settings", json={"labels": ["crack"]})
    assert patch.status_code == 200
    ...
    payload = activate_lora.json()["data"]["activation"]["runtime_sync"]
    assert payload["mode"] == "local_managed_vllm"
    assert payload["status"] == "synced"
    assert payload["actions"][0]["action"] == "start"
    assert payload["actions"][1]["action"] == "load"
