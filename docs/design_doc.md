...
| 像素级分割 | SAM 2 / SAM 2.1（facebookresearch/sam2） |
| 推理引擎 | vLLM |
| 微调框架 | LLaMA-Factory |
| 后端框架 | FastAPI |
...
| 数据库存储 | 归一化 0~1 |
| SAM / SAM2 输入 | 像素坐标（后端调用前乘以图片实际宽高转换） |
...
| `services/sam_service.py`：SAM 单例加载，封装 `set_image` / `predict` + “当前图” embedding 缓存 |
...
// Response 200
{
  ...
  "data": {
    ...
    "model_profile": "auto",
    "sam": { "checkpoint": "sam2", "device": "cuda" },
...
```jsonc
{
  ...
  "sam": {
    "checkpoint": "sam2",                // 默认 symbolic alias；真实联调时可替换为本地 .pt 或启用公开下载
...
- `dev_6gb`：固定小模型（如 Qwen3-VL 2B + SAM2 stub / CPU），优先保证开发与联调可持续。
- `demo_32gb`：固定大模型（如 Qwen3-VL 8B + SAM2.1 real stack），追求效果。
