...
- `POST /api/finetune/start` 当前请求体只有 `project_id`，前端无法提交训练轮次、学习率、batch、梯度累计、logging steps 等超参数。
- 当前接口清单里没有“上传 / 导入自定义训练数据集”的入口，前端只能在任务创建后读取 `dataset_path`，不能让用户提交外部 JSONL 或切换到自定义训练集来源。
- 当前接口清单里没有“训练数据集格式校验”入口，前端无法在启动前校验用户自带数据集是否符合微调格式。
- 当前接口清单里没有“暂停 / 取消 / 恢复 / 重试”微调任务的控制接口，前端目前只能启动任务、读取状态日志、激活完成后的 LoRA。
